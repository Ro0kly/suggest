...
private class DisplayLinkProxy4p4 {
    weak var coordinator: ScrollCoordinator4p4?
...
class ScrollCoordinator4p4: NSObject, UIScrollViewDelegate, ObservableObject {
    ...
    private var displayLinkProxy: DisplayLinkProxy4p4?
    ...
    func startAutoScroll() {
        ...
        let proxy = DisplayLinkProxy4p4()
        proxy.coordinator = self
        displayLinkProxy = proxy
        displayLink = CADisplayLink(target: proxy, selector: #selector(DisplayLinkProxy4p4.tick))
...
struct ChipRowView4p4: View {
    let chips: [Chip4p4]
    let index: Int
    let coordinator: ScrollCoordinator4p4
...
struct ContentView4point4: View {
    ...
    @StateObject private var coordinator = ScrollCoordinator4p4()
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ForEach(Array(rows.enumerated()), id: \.offset) { index, chips in
                    ChipRowView4p4(chips: chips, index: index, coordinator: coordinator) { chip in
